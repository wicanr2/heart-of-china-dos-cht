...
- [x] **macOS**：`.github/workflows/build.yml`（macos-14 runner clone scummvm@f4526cf + 套 patch + 產資產 + dylibbundler `.app`）。**CI run 成功**（patch 套用 + dgds build + `.app` 打包，7m53s），artifact `hoc-cht-macos`。
