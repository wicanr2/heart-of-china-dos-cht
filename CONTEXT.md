...
## 譯名表（character / proper-noun glossary）— 民初冒險通俗派

翻譯時一律使用，確保全劇本一致。`✓` = 定案，`?` = 草稿待全劇本校對定案。
說話人標籤格式：`LUCKY:` → `老馬：`（全形冒號）；名牌鍵大寫對齊。

| 英文 | 中文 | 出現 | 備註 |
|---|---|---|---|
| Lucky（Jake "Lucky" Masters）✓ | **老馬** | 1513 | 主角，吊兒郎當的美國冒險飛行員。全名傑克·馬斯特斯；綽號「老馬」帶江湖味 |
| Chi（趙奇）✓ | **趙奇** | 552 | 中國武術家夥伴，道地中文名 |
| Kate（Kate Lomax）✓ | **凱特** | 552 | 被綁架的報業千金，後成戀人 |
| Lomax（E.A. Lomax）? | 羅麥斯 | 139 | 凱特之父，報業富商 |
| Li Deng / Deng ? | 李鄧（軍閥）| 18 | 反派軍閥 |
...
| Acayib ? | 阿賈伊 | 22 | |
| Nalini ? | 娜里妮 | 19 | |
| Yankee Eagle ? | 洋基之鷹（號）| — | 老馬的飛機 |

地名（待翻譯時定案）：中國（成都/上海/香港）→ 尼泊爾（加德滿都）→ 印度 → 伊斯坦堡。

## Flagged ambiguities

- 主角綽號「老馬」vs 正名「傑克」：旁白/正式場合用「傑克」、江湖口吻用「老馬」，依語境，
  翻譯時保持一致原則（待全劇本校對收斂）。
- 尼泊爾/印度/土耳其配角音譯 `?` 項，於翻譯該段落時逐一定案，回填本表。
- `Li Deng` 反派：歷史聯想敏感，採中性音譯「李鄧」+「軍閥」職稱，不另影射。
